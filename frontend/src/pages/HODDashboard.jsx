...
const HODDashboard = () => {
    ...
    const [activeTab, setActiveTab] = useState('analytics'); // 'analytics', 'approvals', 'reports'
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            // HOD uses different endpoints
            const activitiesRes = await api.get('/activities/department');

            // For now, HOD analytics are derived from their department activities natively
            const activities = activitiesRes.data;
            const pending = activities.filter(a => a.status === 'Pending').length;
            const approved = activities.filter(a => a.status === 'Approved').length;

            // Calculate total points granted in this department
            const totalScore = activities.reduce((sum, act) => {
                return act.status === 'Approved' ? sum + (act.assigned_score || 0) : sum;
            }, 0);
    ...
    const handleReview = async (activityId, status, score) => {
        ...
        try {
            await api.put(`/activities/${activityId}/review`, {
                ...
                review_comments: `Reviewed by HOD: ${status}`
            });
            fetchData();
        } catch (err) {
    ...
    const exportToCSV = () => {
        if (departmentActivities.length === 0) return alert('No data to export');

        const headers = ['ID', 'Faculty Name', 'Activity Title', 'Category', 'Date', 'Status', 'Assigned Score'];
        const csvRows = [headers.join(',')];

        departmentActivities.forEach(acc => {
            const row = [
                acc.id,
    ...
    if (loading && !analytics) return <div style={{ padding: '2rem' }}>Loading Department Analytics...</div>;

    const pendingActivities = departmentActivities.filter(a => a.status === 'Pending');
        ...
        <div className="app-container">
            <aside style={{ width: '250px', background: 'var(--surface)', borderRight: '1px solid var(--border)', padding: '2rem', display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '2rem' }}>
                    ...
                    <h2 style={{ color: 'var(--primary)', margin: 0 }}>HOD Portal</h2>
                </div>
            ...
            <main className="main-content">
                <header style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        <h1>{activeTab === 'analytics' && 'Department Analytics'}
                            {activeTab === 'approvals' && 'Pending Approvals'}
                            {activeTab === 'reports' && 'Activity Reports'}
                        </h1>
                        <p style={{ color: 'var(--text-muted)' }}>Manage your department faculty submissions.</p>
                    </div>
                    {activeTab === 'reports' && (
                    ...
                    <>
                        <div className="metrics-grid">
                            <div className="card metric-card">
                                ...
                                <div className="metric-value">{analytics?.departmentScore}</div>
                                <div className="metric-label">Total API Granted</div>
                            </div>
                            <div className="card metric-card">
                                <div style={{ background: 'rgba(16, 185, 129, 0.1)', padding: '0.75rem', borderRadius: '0.5rem', color: 'var(--success)' }}>
                                    <CheckCircle size={24} />
                                </div>
                                <div className="metric-value" style={{ color: 'var(--success)' }}>{analytics?.pendingReviews}</div>
                            ...
                            <div className="card metric-card">
                                ...
                                <div className="metric-value" style={{ color: 'var(--danger)' }}>{analytics?.totalSubmissions}</div>
